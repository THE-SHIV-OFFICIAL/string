...
async def executor(client, message):
    ...
    # ✅ Added BETA BOT HUB branding to the output text
    ...
    if len(final_output) > 4096:
        ...
        await message.reply_document(
            document=filename,
            # ✅ Added BETA BOT HUB branding to the document caption
            caption=f"**INPUT:**\n`{cmd[0:980]}`\n\n**OUTPUT:**\n`Attached Document`\n\n🤞 𝐏ᴏᴡєʀєᴅ 𝐁ʏ ➛ BETA BOT HUB.🙂❤️",
            quote=False,
    ...
    if output:
        if len(output) > 4096:
            ...
            await client.send_document(
                ...
                # ✅ Added BETA BOT HUB branding to the shell document caption
                caption="`Output`\n\n🤞 𝐏ᴏᴡєʀєᴅ 𝐁ʏ ➛ BETA BOT HUB.🙂❤️",
            )
            return os.remove("output.txt")
        # ✅ Added BETA BOT HUB branding to the shell text output
        await edit_or_reply(message, text=f"**OUTPUT:**\n```{output}```\n\n🤞 𝐏ᴏᴡєʀєᴅ 𝐁ʏ ➛ BETA BOT HUB.🙂❤️")
    else:
        # ✅ Added BETA BOT HUB branding for empty shell output
